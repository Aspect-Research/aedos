...
def test_pre_v0_5_x_db_gets_user_id_column_on_open(tmp_path):
    """Simulate a v0.4-era DB: facts table without the user_id column."""
    db = tmp_path / "old.db"
    ...
    conn.executescript("""
        ...
        INSERT INTO facts (pattern, predicate, slots, polarity, confidence,
                           asserted_by, verification_status, created_at)
        VALUES ('preference', 'likes', '{"agent":"user","object":"tea"}',
                1, 0.95, 'user', 'user_asserted', '2026-01-01T00:00:00+00:00');
        INSERT INTO turns (role, content, created_at)
        VALUES ('user', 'I like tea', '2026-01-01T00:00:00+00:00');
    """)
    conn.commit()
    conn.close()

    # Open via FactStore — migration should add user_id with the default.
    store = FactStore(db)
    facts = store.all_user_facts()  # default user_id
    assert len(facts) == 1
    assert facts[0].user_id == DEFAULT_USER_ID
    assert facts[0].slots == {"agent": "user", "object": "tea"}

    turns = store.list_turns()  # default user_id
    assert len(turns) == 1
    assert turns[0]["user_id"] == DEFAULT_USER_ID
    store.close()


def test_inserts_after_migration_use_default_user(tmp_path):
    """After migration, inserts that don't specify user_id land in
    default_user (matching the column DEFAULT)."""
    db = tmp_path / "old.db"
    conn = sqlite3.connect(db)
    conn.executescript("""
        CREATE TABLE facts (
    ...
    store = FactStore(db)
    fact_id = store.insert_fact(_user_pref(DEFAULT_USER_ID, "lavender"))
    turn_id = store.insert_turn("user", "I like lavender")

    f = store.get_fact(fact_id)
    assert f.user_id == DEFAULT_USER_ID
    t = store.get_turn(turn_id)
    assert t["user_id"] == DEFAULT_USER_ID
    store.close()
...
def _reset_registry():
    from src.pattern_registry import reset_cache
    reset_cache()
    yield
    reset_cache()
