...
def test_v090_parallel_verify_overlaps_wall_clock(tmp_path):
    ...
    # Sequential would be ~600ms search + ~100ms+ fsync overhead per
    # claim's pipeline_events writes (Windows SQLite); parallel with
    # 3 workers compresses search to ~200ms and lets fsyncs overlap.
    # The threshold has been padded twice: 0.55 (v0.9.0 baseline) →
    # 0.90 (Phase 9a) after observing the test go flaky when the full
    # legacy suite was running concurrently on Windows. The proof-of-
    # overlap signal is "well under sequential", not "exactly the
    # parallel lower bound" — sequential on a busy machine can reach
    # 1.5s+, so 0.90 still cleanly discriminates.
    assert elapsed < 0.90, (
        f"verification took {elapsed:.2f}s — expected parallel dispatch "
        f"to finish under 0.90s with 200ms search latency × 3 claims"
    )
...
def test_router_routes_external_data_claim_to_retrieval_directly(tmp_path):
    ...
    """
    from src.legacy.verifiers.retrieval_verifier import RetrievalVerifier, Snippet
    from src.legacy.verifiers.code_generation import CodeGenerationVerifier
