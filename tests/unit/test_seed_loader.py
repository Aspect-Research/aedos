...
class TestSeedAliasDeletions:
    ...
    def test_seed_count_is_71_after_deletions_t1_ws2_and_ws3b_additions(self, seeds):
        ...
        # v0.16.5 added 2 height-comparison python rows (taller_than, shorter_than;
        # premise_properties P2048) = 81.
        assert len(seeds) == 81, (
            f"expected 81 seed rows (62 after the 21 alias deletions + 6 T1 "
            ...
            f"string-count python rows + 2 v0.16.5 height-comparison rows), "
            f"got {len(seeds)}"
        )
...
class TestSeedT1IntervalEndpoints:
    ...
    def test_endpoint_rows_load_without_tripping_loader(self, tmp_path):
        ...
        assert n == 81  # +2 v0.16.3 capital pins, +6 v0.16.4 string-count, +2 v0.16.5 taller/shorter
