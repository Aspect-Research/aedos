...
DEFAULT_MODEL_BY_PURPOSE: dict[str, dict] = {
    ...
    # v0.16.4: the final constrained-rewrite editor that folds the per-claim
    # verdicts into the reply (correct/remove/caveat) once every claim is
    # verified. A bounded editing pass — same fast model as the draft `chat`.
    "chat:revise":                      {"model": "claude-haiku-4-5", **_ANTHROPIC},
