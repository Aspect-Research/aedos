...
from src.legacy.corrector import Corrector, Intervention
...
from src.legacy.router import Decision, Router, RoutingOutcome
...
Practical implication: commit to specific, verifiable answers instead of disclaiming. The pipeline will verify your answer and gently surface a correction if you're off, so a concrete-but-wrong answer is more useful than a refusal — it gives the verifier something to check, and the user gets the right value either way.
...
class Pipeline:
    ...
    def _stage_user_side(self, user_message: str):
        ...
        # v0.14 Phase 8.6: pass the raw user_message as raw_text so the
        # session-marker detector sees the original utterance rather than
        # the extractor's projected source_text (which may strip "Let's
        # say for this conversation" and similar markers).
        user_decisions: list[Decision] = [
            self.router.route(
                c, origin="user", source_turn_id=user_turn_id,
    ...
    def _build_chat_system_prompt(
        ...
        from src.legacy.router.constants import is_self_attribute as _is_self
        # Bucket stored facts.
...
def build_pipeline(
    ...
    *,
    ...
    from src.legacy.pattern_registry import load_default_registry
    from src.legacy.verifiers.code_generation import CodeGenerationVerifier
