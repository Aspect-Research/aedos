"""Pattern registry (v0.5).

Loads patterns.yaml and exposes lookup + prompt-formatting helpers. The
pattern catalog is bounded (8 patterns); predicate labels within a
pattern are free-form.

(v0.5) Patterns no longer carry verification routing — the LLM router
(``src/llm_router.py``) decides per-claim. The pattern's role is purely
structural classification: it shapes extraction, defines slot identity
for store lookups, and supplies a query strategy for retrieval when
...
_DEFAULT_PATH = Path(__file__).resolve().parent.parent.parent / "patterns.yaml"
