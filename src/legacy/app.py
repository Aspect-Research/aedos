...
from src.legacy.pipeline import Pipeline, build_pipeline
from src.legacy.pattern_registry import load_default_registry
# v0.14 cutover (Phase 9c): the v2 sub-app mount that previously
# lived here was removed because v2 is now the top-level
# ``src.app``. Phase 9d's mount swap restructures the topology.
# Operators wanting hot rollback to v1 should run this app directly
# (``python -m src.legacy.app``); v2 hot-mount under /legacy is
# v0.15 work.
...
# Phase 9c rename: the /v2 sub-app mount was removed because v2 is
# now the top-level ``src.app``. Phase 9d will redo the mount topology.
