...
from src.legacy.llm_router import ROUTING_METHODS, RoutingDecision, route_claim
from src.legacy.pattern_registry import Pattern, PatternRegistry
...
from src.legacy.verifiers.code_generation import (
...
class Router:
    ...
    def route(
        self, claim: dict, origin: str, source_turn_id: int,
        *, raw_text: str | None = None,
    ) -> Decision:
        """Route one claim. ``raw_text`` (v0.14 Phase 8.6) is the
        original turn text — the user's actual message, before
        extraction's source_text projection. The session-marker
        detector reads it (in ``_store``) to decide session-local vs
        cross-session storage. When None (default; tests, direct
        dispatch-helper calls), the marker check falls back to
        ``claim['source_text']`` — the pre-Phase-8.6 behavior. The
        pipeline's stage methods pass the raw turn text explicitly so
        a user message ``"Let's say for this conversation I live in
        Williamsburg"`` is recognized as session-scoped even when the
        extractor strips the marker phrase from ``source_text``."""
        ...
        try:
            if origin == "user":
                return self._route_user(
                    claim, pattern, source_turn_id, raw_text=raw_text,
                )
    ...
    def _route_user(
        ...
        *, raw_text: str | None = None,
        ...
        from src.legacy.router.constants import is_self_attribute
        if not is_self_attribute(claim):
            return self._route_user_world_claim(
                claim, pattern, source_turn_id, raw_text=raw_text,
            )
    ...
    def _route_model(
        ...
        *, raw_text: str | None = None,
        ...
        if anomaly is not None:
            decision = self._route_routing_anomaly(
                claim, source_turn_id, anomaly, raw_text=raw_text,
            )
        ...
        decision = self._dispatch_method(
            claim, pattern, source_turn_id, routing, raw_text=raw_text,
        )
    ...
    def _dispatch_method(
        ...
        *,
        ...
        if method == "unverifiable":
            return self._route_unverifiable(
                claim, source_turn_id, raw_text=raw_text,
            )
        raise RuntimeError(
    ...
    def _store(
        ...
        *,
        ...
        #
        # v0.14 Phase 8.6: marker detection now runs on ``raw_text`` when
        # provided (the raw turn text the pipeline passed to ``route()``)
        # and falls back to ``claim['source_text']`` otherwise. The
        # extractor's ``source_text`` projection often strips the marker
        # phrase ("Let's say for this conversation I live in Williamsburg"
        # → source_text "I live in Williamsburg"); pre-Phase-8.6 the
        # marker check then missed and the fact got stored cross-session.
        # The pipeline now threads the raw user_message through; tests
        # and direct dispatch-helper callers that don't pass ``raw_text``
        # get the legacy source_text-based behavior unchanged.
        from src.legacy.session_markers import is_session_scoped
        marker_text = raw_text if raw_text is not None else claim.get("source_text")
        session_id = None
        if (asserted_by == "user"
            and self.session_id is not None
            and is_session_scoped(marker_text)):
